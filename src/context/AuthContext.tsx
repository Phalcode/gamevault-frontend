...
function computeNextTokenRefresh(token: string): Date {
  ...
  if (!payload) {
    console.warn(
      "[Auth] Unable to parse JWT payload; scheduling immediate refresh.",
    );
    return new Date();
  }
  ...
  if (
    ...
    console.log(
      `[Auth] Parsed token exp only exp=${exp} -> next refresh @ ${next.toISOString()}`,
    );
  } else {
    console.warn(
      "[Auth] JWT payload missing exp/creation; scheduling immediate refresh.",
    );
    next = new Date();
  }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function refreshWithToken(refreshToken: string): Promise<AuthTokens> {
    ...
    if (!res.ok) {
      const text = await res.text();
      throw new Error(
        `Refresh failed (${res.status}): ${text || res.statusText}`,
      );
    }
    return res.json();
  }
  ...
  async function fetchCurrentUser(): Promise<User> {
    ...
    if (!res.ok) {
      const text = await res.text();
      throw new Error(
        `GET /api/users/me failed (${res.status}): ${text || res.statusText}`,
      );
    }
    return res.json();
  }
  ...
  const performRefresh = useCallback(async () => {
    ...
    console.log(
      "[Auth] Refresh success. Next @",
      nextTokenRefreshRef.current.toISOString(),
    );
  }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.warn(
          "[Auth] Silent restore failed; clearing stored tokens.",
          e,
        );
