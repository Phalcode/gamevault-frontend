...
interface AuthContextValue {
  ...
  user: User | null
  error: string | null
  loading: boolean              // explicit login in progress
  bootstrapping: boolean        // initial silent restore in progress
...
function computeNextTokenRefresh(token: string): Date {
  ...
  if (!payload) {
    console.warn('[Auth] Unable to parse JWT payload; scheduling immediate refresh.')
    return new Date()
  }

  const exp = payload.Exp ?? payload.exp
  const creation = payload.Creation ?? payload.creation ?? payload.iat ?? payload.Iat
  const now = Date.now()
  let next: Date

  if (typeof exp === 'number' && typeof creation === 'number' && exp > creation) {
    const lifetimeSec = exp - creation
    next = new Date(now + lifetimeSec * 1000)
    console.log(
      `[Auth] Parsed token exp=${exp} creation=${creation} lifetimeSec=${lifetimeSec} -> next refresh @ ${next.toISOString()}`
    )
  } else if (typeof exp === 'number') {
    next = new Date(exp * 1000)
    console.log(`[Auth] Parsed token exp only exp=${exp} -> next refresh @ ${next.toISOString()}`)
  } else {
    console.warn('[Auth] JWT payload missing exp/creation; scheduling immediate refresh.')
    next = new Date()
  }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [bootstrapping, setBootstrapping] = useState(true) // initial silent attempt
  ...
  async function refreshWithToken(refreshToken: string): Promise<AuthTokens> {
    console.log('[Auth] Performing refresh with stored token...')
  ...
  async function fetchCurrentUser(): Promise<User> {
    ...
    if (!res.ok) {
      const text = await res.text()
      throw new Error(`GET /api/users/me failed (${res.status}): ${text || res.statusText}`)
    }
    return res.json()
  }
  ...
  const isTokenNearExpiry = useCallback(() => {
    ...
    const near = nxt.getTime() <= threshold
    console.log(
      `[Auth] Token check: now=${new Date().toISOString()} next=${nxt.toISOString()} nearExpiry=${near}`
    )
    return near
  }, [])

  const performRefresh = useCallback(async () => {
    const refreshToken = authRef.current?.refresh_token || localStorage.getItem(REFRESH_KEY)
    if (!refreshToken) {
    ...
    console.log('[Auth] Refresh success. Next @', nextTokenRefreshRef.current.toISOString())
  }, [])
  ...
  const ensureFreshToken = useCallback(async () => {
    ...
    refreshInFlightRef.current = (async () => {
      try {
        ...
        console.error('[Auth] Refresh failed; forcing logout.', e)
        logout()
      } finally {
  ...
  const authFetch = useCallback(async (input: string, init?: RequestInit) => {
    ...
    console.log(`[AuthFetch] ${init?.method || 'GET'} ${input}`)
    const res = await fetch(input, { ...(init || {}), headers })
    console.log(`[AuthFetch] ${res.status} ${input}`)
    return res
  }, [ensureFreshToken])

  /* ---------------- Manual login ------------------------------- */
  const loginBasic = useCallback(async ({ server, username, password }: LoginArgs) => {
    setError(null)
    setLoading(true)
    setUser(null)
    setAuth(null)
    ...
    try {
      if (!server || !username || !password) throw new Error('All fields are required.')
      ...
      setUser(me)
      console.log('[Auth] Manual login succeeded.')
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!storedRefresh || !storedServer) {
        console.log('[Auth] No stored refresh token/server; showing login.')
        setBootstrapping(false)
        return
      }
      ...
      setServerUrl(serverRef.current)
      try {
        console.log('[Auth] Attempting silent restore...')
        ...
        console.warn('[Auth] Silent restore failed; clearing stored tokens.', e)
  ...
  const logout = useCallback(() => {
    ...
    setError(null)
    localStorage.removeItem(REFRESH_KEY)
    // Keep server URL so user does not retype; comment next line to keep
    // localStorage.removeItem(SERVER_KEY)
  }, [])
