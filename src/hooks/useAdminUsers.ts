...
export function useAdminUsers(): UseAdminUsersResult {
  ...
  const fetchUsers = useCallback(async (force = false) => {
    ...
    setError(null)
    try {
      const res = await authFetch(`${safeBase}/api/users`, { headers: { Accept: 'application/json' } })
      if (!res.ok) {
        const text = await res.text()
        throw new Error(`Fetching users failed (${res.status}): ${text || res.statusText}`)
      }
  ...
  const toggleActivated = async (u: User) => {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(`Activation update failed (${res.status}): ${text || res.statusText}`)
      }
      ...
      console.error('[toggleActivated]', e)
      setUsers(prev => prev.map(x =>
        (x.id ?? (x as any).ID) === uid
          ? { ...x, activated: current }
          : x
  ...
  const deleteUser = async (u: User) => {
    ...
    try {
      ...
      let updated: any = null
      if (res.status !== 204) { try { updated = await res.json() } catch {} }
      setUsers(prev => prev.map(x => {
        if ((x.id ?? (x as any).ID) === uid) {
          if (updated && ('deleted_at' in updated || 'DeletedAt' in updated)) return normalizeUser({ ...x, ...updated })
  ...
  const recoverUser = async (u: User) => {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text()
        throw new Error(`Recover failed (${res.status}): ${text || res.statusText}`)
      }
      let updated: any = null
      try { updated = await res.json() } catch {}
      setUsers(prev => prev.map(x => {
  ...
  const updateUser = async (u: User, payload: Partial<User> & { password?: string | null }) => {
    ...
    try {
      ...
      if (payload.password && payload.password.trim().length) body.password = payload.password
      ...
      if (!res.ok) {
        const text = await res.text()
        return { ok: false, message: `Save failed (${res.status}): ${text || res.statusText}` }
      }
      const updated = await res.json()
      setUsers(prev => prev.map(x => ((x.id ?? (x as any).ID) === id ? normalizeUser({ ...x, ...updated }) : x)))
      return { ok: true }
    } catch (e) {
  ...
  const updateUserRole = async (u: User, newRole: PermissionRole) => {
    ...
    try {
      const res = await authFetch(`${safeBase}/api/users/${uid}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
        body: JSON.stringify({ role: newRole }), // numeric
      })
      if (!res.ok) {
        const txt = await res.text()
        throw new Error(`Role update failed (${res.status}): ${txt || res.statusText}`)
      }
      let updated: any = null
      try { updated = await res.json() } catch {}
      if (updated && typeof updated === 'object') {
