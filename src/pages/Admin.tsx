...
export function AdminPage() {
  ...
  const roleNum = typeof currentUser?.role === 'number'
  ...
  if (!isAdmin) {
    return (
      <div className="flex flex-col items-stretch min-h-0 pt-0">
        <h2 className="m-0 mb-4 text-[1.65rem] font-semibold tracking-wide leading-tight">Admin</h2>
        <div
  ...
  const {
    users, loading, error, refresh, opBusy,
    toggleActivated, deleteUser, recoverUser, updateUser, updateUserRole, setUsers
  } = useAdminUsers()
  ...
  const requiredRegFields = new Set(serverInfo?.required_registration_fields || [])
  ...
  const deletedCount = useMemo(
    () => users.reduce((n, u) => ((u.deleted_at ?? (u as any).DeletedAt) != null ? n + 1 : n), 0),
    [users]
  )
      ...
      >
        <div className="flex flex-wrap items-center gap-4">
          <div>
            ...
            <p className="text-[0.65rem] opacity-60 mt-[2px]">Current role: {roleNum != null ? PermissionRoleLabel[roleNum] : 'unknown'}</p>
          </div>
          <label className="relative inline-flex items-center gap-2 cursor-pointer select-none text-[0.7rem] bg-[rgba(32,31,45,0.5)] px-2.5 pr-3 py-1.5 rounded-xl border border-[#2d2c38]">
            ...
            <span
              className={[
                ...
                showDeleted ? 'bg-[linear-gradient(120deg,#6d63f0,#574ae3)]' : 'bg-[#2c2b38]'
              ].join(' ')}
            >
              <span
                className={[
            ...
            <span className="font-semibold tracking-[.5px] opacity-80 uppercase">Show deleted users</span>
          </label>
          <div className="flex gap-2">
            ...
            >
              <span className={loading ? 'animate-spin' : ''}>
                <MaterialSymbolsLightDirectorySyncRounded width={20} height={20} />
              </span>
            </button>
            <button
              type="button"
          ...
          <span className="text-[0.6rem] opacity-55 tracking-[.5px] ml-auto pr-1">
            Total: {users.length} • Deleted: {deletedCount} • Visible: {filteredUsers.length}
          </span>
        </div>

        {loading && <div className="text-[0.8rem] opacity-75 mt-3">Loading users…</div>}
        {error && !loading && (
        ...
        {!loading && !error && filteredUsers.length === 0 && (
          <div className="text-[0.8rem] opacity-75 mt-3">
            {users.length === 0 ? 'No users returned.' : 'No users match current filter (all are deleted).'}
          </div>
        )}
              ...
              {filteredUsers.map(u => {
                ...
                return (
                  <UserCard
                    ...
                    onChangeRole={(user, newRole) => updateUserRole(user, newRole)}
                  />
                )
              })}
            </ul>
      ...
      {editingUser && (
        <UserEditorModal
          ...
          onSave={async (payload) => {
            ...
            if (res.ok) {
              setEditingUser(prev => prev ? { ...prev, ...payload, password: undefined } : prev)
            }
            return res
          }}
        />
