...
function RoleDropdown({ role, disabled, onChange }: RoleDropdownProps) {
  ...
  const [activeIndex, setActiveIndex] = useState(Math.max(0, ROLE_OPTIONS.findIndex(r => r === role)))
  ...
  const calcPosition = useCallback(() => {
    ...
    const estimatedHeight = panelRef.current?.offsetHeight ?? (ROLE_OPTIONS.length * 34 + 12)
    const spaceBelow = window.innerHeight - rect.bottom
    const spaceAbove = rect.top
    const shouldFlip = spaceBelow < estimatedHeight + 8 && spaceAbove > estimatedHeight + 8
  ...
  const openPanel = useCallback(() => {
    if (disabled) return
    setActiveIndex(Math.max(0, ROLE_OPTIONS.findIndex(r => r === role)))
    setOpen(true)
  }, [disabled, role])
  ...
  const onListKey = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault()
      setActiveIndex(i => (i - 1 + ROLE_OPTIONS.length) % ROLE_OPTIONS.length)
    } else if (e.key === 'Home') {
      e.preventDefault()
      setActiveIndex(0)
    } else if (e.key === 'End') {
  ...
  const btnBase = 'w-full inline-flex items-center justify-between gap-2.5 rounded-[10px] px-3 py-2 text-[0.65rem] font-semibold tracking-wide transition disabled:opacity-45 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40'
  const btnColors = 'bg-[linear-gradient(140deg,#252436,#1f1e2d)] text-[#ececf6] hover:bg-[linear-gradient(140deg,#2b2a3d,#242335)]'
  const btnOpen = 'bg-[linear-gradient(140deg,#302f46,#27263a)] shadow-[0_6px_18px_-8px_rgba(0,0,0,0.55)]'
  return (
    <div className="relative text-[0.65rem] w-[148px]">
      <button
        ref={triggerRef}
        type="button"
        className={[btnBase, btnColors, open && !disabled ? btnOpen : '', disabled ? 'cursor-default' : ''].filter(Boolean).join(' ')}
        disabled={disabled}
      ...
      >
        <span className="flex-1 truncate text-left">{PermissionRoleLabel[role]}</span>
        <span
          ...
          style={open ? { transform: 'rotate(180deg)', color: 'var(--color-accent,#6459DF)' } : undefined}
          aria-hidden="true"
        >
      ...
      {open && coords && createPortal(
        ...
        >
          {ROLE_OPTIONS.map((r, idx) => {
            ...
            const base = 'relative flex items-center gap-2 px-2.5 py-2 text-[0.65rem] font-semibold tracking-wide rounded-lg cursor-pointer select-none transition-colors'
            const hover = 'hover:bg-[linear-gradient(145deg,#2c2b3e,#262536)] hover:text-white'
            const sel = 'bg-[linear-gradient(145deg,#443d72,#352f5a)] text-white shadow-[0_0_0_1px_rgba(100,89,223,0.55),0_6px_20px_-10px_rgba(100,89,223,0.45)]'
            const act = 'bg-[linear-gradient(145deg,#34324c,#2a293d)] text-white'
            return (
              <li
                id={`role-opt-${r}`}
                key={r}
                ...
                data-index={idx}
                className={[base, hover, selected && sel, !selected && active && act].filter(Boolean).join(' ')}
                onMouseEnter={() => setActiveIndex(idx)}
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => commit(idx)}
              >
                <span className="w-4 h-4 flex items-center justify-center text-transparent transition-colors" aria-hidden="true" style={selected ? { color: 'var(--color-accent,#6459DF)' } : undefined}>
                  {selected && (
                ...
                <span
                  ...
                  style={{ background: 'radial-gradient(circle at 20% 15%,rgba(100,89,223,0.25),transparent 70%),linear-gradient(160deg,rgba(100,89,223,0.22),rgba(100,89,223,0))', mixBlendMode: 'overlay', opacity: selected ? 0.55 : undefined }}
                  aria-hidden="true"
                />
              </li>
            )
...
export function UserCard({
  ...
  const isActive = activeRaw === true || activeRaw === 1 || activeRaw === 'activated'
  const role = (typeof user.role === 'number' ? user.role : PermissionRole.GUEST) as PermissionRole
  ...
  return (
    <li
      className={[
        ...
        isDeleted ? 'opacity-55 border-[#3a2d2d]' : 'border-[#2c2b38] hover:border-accent/50'
      ].join(' ')}
      title={isDeleted ? `Deleted at: ${del}` : ''}
    >
      <UserAvatarMini media={user.avatar} serverUrl={serverUrl} />
      <div style={{ ...styles.meta, flex: 1 }}>
        <span style={styles.username}>{user.username || (user as any).Username || '(no username)'}</span>
        ...
        <div style={styles.activationRow}>
          ...
          <div className="flex flex-col gap-1 text-[0.55rem] pt-1">
            <span className="uppercase font-semibold tracking-wide text-[#cfcfe0] opacity-65 pl-0.5">Role</span>
            <RoleDropdown
              role={role}
              disabled={busy || editing || isDeleted}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  username: {
    fontWeight: 600, fontSize: '.9rem', color: '#e5e5ef', overflow: 'hidden', textOverflow: 'ellipsis',
    whiteSpace: 'nowrap', maxWidth: 140,
  },
  id: { fontSize: '.65rem', opacity: .55, marginTop: 2, letterSpacing: '.5px' },
  deletedBadge: {
    marginTop: 4,
    fontSize: '.55rem',
    fontWeight: 700,
  ...
  inlineToggle: { position: 'relative', width: 32, height: 16, borderRadius: 20, display: 'inline-block', transition: 'background .25s ease' },
  inlineKnob: {
    position: 'absolute', top: 2, width: 12, height: 12, background: '#fff', borderRadius: '50%',
    boxShadow: '0 2px 4px rgba(0,0,0,.4)', transition: 'transform .25s cubic-bezier(.4,.2,.2,1)',
  },
  inlineToggleLabel: {
    fontWeight: 600,
    letterSpacing: '.6px',
...
const deleteButtonStyle = (disabled: boolean): React.CSSProperties => ({
  ...
  height: 32,
  borderRadius: 12,
  background: disabled ? 'rgba(120,53,53,0.35)' : 'linear-gradient(120deg,#643434,#7b3f3f)',
...
const recoverButtonStyle = (disabled: boolean): React.CSSProperties => ({
  ...
  height: 32,
  borderRadius: 12,
  background: disabled ? 'rgba(53,81,53,0.35)' : 'linear-gradient(120deg,#2f6d3d,#3c8a4e)',
