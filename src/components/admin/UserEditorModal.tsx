...
export function UserEditorModal({ user, onClose, onSave, onUserUpdated }: Props) {
  ...
  const [avatarImg, setAvatarImg] = useState<ImageState>({
    file: null, via: 'none', preview: null, urlInput: '', original: null, loadedId: undefined
  })
  ...
  useEffect(() => () => {
    revokeRef.current.forEach(u => { try { URL.revokeObjectURL(u) } catch {} })
  }, [])

  const avatarMediaId =
    (user.avatar as any)?.ID ??
    (user.avatar as any)?.id ??
    (user as any).avatar_id

  const backgroundMediaId =
    (user.background as any)?.ID ??
    (user.background as any)?.id ??
    (user as any).background_id
  ...
  const saveImages = async () => {
    ...
    try {
      ...
      if (avatarId || backgroundId) {
        ...
        const res = await authFetch(`${base}/api/users/${uid}`, {
          method: 'PUT',
    ...
    <div style={styles.overlay}>
      <div style={styles.modal} role="dialog" aria-modal="true" aria-labelledby="user-settings-title">
        <div style={styles.header}>
          <h3 id="user-settings-title" style={{ margin: 0, fontSize: '1.05rem', letterSpacing: '.5px' }}>
            User Settings
          </h3>
          <button
            type="button"
        ...
        <div style={styles.tabBar}>
          <button type="button" onClick={() => setActiveTab('images')} style={tabButtonStyle(activeTab === 'images')}>
            Edit Images
          </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 28, padding: '2px 4px 12px' }}>
              <div style={imageStyles.zonesRow}>
                <div
                  ...
                  style={imageZoneStyle(avatarImg.preview)}
                >
                  ...
                  >
                    {avatarImg.preview
                      ? <img src={avatarImg.preview} alt="Avatar preview" style={imageStyles.previewImg} draggable={false} />
                      : <div style={imageStyles.emptyMsg}><span>{avatarMediaId ? 'Loading…' : 'Drag & Drop / Click / Paste'}</span></div>
                    }
                  </div>
                  <div style={imageStyles.urlBoxWrap}>
                    <input
                      ...
                      onChange={e => setAvatarImg(p => ({ ...p, urlInput: e.target.value }))}
                      onPaste={() => {/* handled by container */}}
                      style={styles.input}
                    />
                    {avatarImg.preview && avatarImg.preview !== avatarImg.original && (
                      <button
                        ...
                        onClick={() => setAvatarImg(p => ({
                          ...p, file: null, via: 'none', preview: p.original, urlInput: ''
                        }))}
                      >
                        Reset
                      </button>
                ...
                <div
                  ...
                  onDragOver={handleDragOver}
                  style={imageZoneStyle(bgImg.preview)}
                >
                  <div style={imageStyles.zoneHeader}>
                  ...
                  <div
                    style={{ ...imageStyles.previewWrap, height: 220 }}
                  ...
                  >
                    {bgImg.preview
                      ? <img src={bgImg.preview} alt="Background preview" style={{ ...imageStyles.previewImg, objectFit: 'cover' }} draggable={false} />
                      : <div style={imageStyles.emptyMsg}><span>{backgroundMediaId ? 'Loading…' : 'Drag & Drop / Click / Paste'}</span></div>
                    }
                  </div>
                  <div style={imageStyles.urlBoxWrap}>
                    <input
                      ...
                      onChange={e => setBgImg(p => ({ ...p, urlInput: e.target.value }))}
                      style={styles.input}
                    />
              ...
              <div style={imageStyles.footerRow}>
                <div style={{ flex: 1 }}>
                  {imagesMsg && (
                    <div
                      style={{
                        fontSize: '.65rem',
                        color: imagesMsg.toLowerCase().includes('success') ? '#6fe39e' : '#ffb3b3',
                      }}
                    >
                      {imagesMsg}
                    </div>
                ...
                <button
                  ...
                  style={saveButtonStyle(!imagesDirty || savingImages)}
                >
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit() }} style={styles.formGrid}>
              ...
              <div style={styles.formFooterRow}>
                <div style={{ flex: 1 }}>
                  {saveMsg && (
                    <div
                      style={{
                        fontSize: '.65rem',
                        color: saveMsg.startsWith('Successfully') ? '#6fe39e' : '#ffb3b3',
                        paddingLeft: 2
                      }}
                    >
                      {saveMsg}
                ...
                <button
                  ...
                  style={saveButtonStyle(!hasChanges || saving)}
                >
                  {saving ? 'Saving…' : 'Save'}
                </button>
              </div>
...
const styles: Record<string, React.CSSProperties> = {
  overlay: {
    position: 'fixed', inset: 0, background: 'rgba(10,10,18,0.65)',
    backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center',
    justifyContent: 'center', padding: '4vh 3vw', zIndex: 400,
    overflowY: 'auto', overflowX: 'hidden',
  },
  modal: {
    width: '860px', maxWidth: '96vw',
    background: 'linear-gradient(155deg,#2b2a3c 0%,#222130 55%,#1c1b28 100%)',
    border: '1px solid #38384a', borderRadius: 24,
    boxShadow: '0 10px 48px -12px rgba(0,0,0,.75), 0 2px 6px -2px rgba(0,0,0,.6)',
    padding: '28px 34px 32px', position: 'relative',
    color: '#e8e8f4', display: 'flex', flexDirection: 'column',
  ...
  tabBar: {
    display: 'flex', gap: 12, padding: '4px 2px 14px 2px',
    borderBottom: '1px solid #323244', flex: '0 0 auto',
  },
  scrollArea: {
    marginTop: 18, overflowY: 'auto', overflowX: 'hidden', flex: 1,
    display: 'flex', flexDirection: 'column', gap: 24,
    padding: '0 4px 0 2px', scrollbarWidth: 'thin',
  },
  formGrid: {
    display: 'grid',
    gridTemplateColumns: 'repeat(auto-fit, minmax(270px, 1fr))',
    gap: '28px 48px', alignItems: 'start',
    padding: '2px 6px 4px 2px', boxSizing: 'border-box',
  },
  fieldCell: { display: 'flex', flexDirection: 'column' },
  label: {
    display: 'flex', flexDirection: 'column', gap: 8,
    fontSize: '.62rem', letterSpacing: '.55px', fontWeight: 600, color: '#bfbfd3',
  },
  labelText: { paddingLeft: 2, textTransform: 'uppercase', opacity: .9 },
  input: {
...
const tabButtonStyle = (active: boolean): React.CSSProperties => ({
  flex: '0 0 auto',
  background: active
    ? 'linear-gradient(120deg,#5c53d8,#7169ff)'
    : 'linear-gradient(120deg,#2f2e42,#2a293a)',
...
const imageStyles: Record<string, React.CSSProperties> = {
  ...
  previewWrap: {
    ...
    background: 'linear-gradient(145deg,#252432,#1f1e29)',
    height: 160,
    overflow: 'hidden',
    cursor: 'pointer',
    display: 'flex',
...
const imageZoneStyle = (hasPreview: string | null): React.CSSProperties => ({
  ...
  background: 'linear-gradient(150deg,#2a293b,#252432)',
  border: '1px solid #38384c',
  padding: '18px 20px 20px',
  borderRadius: 22,
  position: 'relative',
  minHeight: 270,
  boxShadow: '0 4px 14px -6px rgba(0,0,0,.55)',
  ...(hasPreview ? {} : { outline: '1px dashed rgba(120,120,150,.15)' })
})

const miniBtnStyle = (disabled: boolean): React.CSSProperties => ({
  background: disabled
    ? 'linear-gradient(120deg,#3b3b52,#353448)'
    : 'linear-gradient(120deg,#5c53d8,#6e65f0)',
