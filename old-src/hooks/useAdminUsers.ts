    ...
    async (force = false) => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `Fetching users failed (${res.status}): ${text || res.statusText}`,
          );
        }
  ...
  const toggleActivated = async (u: User) => {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        throw new Error(
          `Activation update failed (${res.status}): ${text || res.statusText}`,
        );
      }
  ...
  const recoverUser = async (u: User) => {
    ...
    try {
      ...
      if (!res.ok) {
        const text = await res.text();
        throw new Error(
          `Recover failed (${res.status}): ${text || res.statusText}`,
        );
      }
      let updated: any = null;
      try {
  ...
  const updateUser = async (
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return {
          ok: false,
          message: `Save failed (${res.status}): ${text || res.statusText}`,
        };
      }
  ...
  const updateUserRole = async (u: User, newRole: PermissionRole) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Role update failed (${res.status}): ${txt || res.statusText}`,
        );
      }
      let updated: any = null;
      try {
